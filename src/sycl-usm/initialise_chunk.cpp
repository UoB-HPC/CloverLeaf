...
void initialise_chunk(const int tile, global_variables &globals) {
  ...
  clover::par_ranged1(globals.context.queue, Range1d{0u, xrange}, [=](int j) {
    field.vertexx[j] = xmin + dx * (static_cast<int>(j) - 1 - x_min);
    field.vertexdx[j] = dx;
  });

  clover::par_ranged1(globals.context.queue, Range1d{0u, yrange}, [=](int k) {
  ...
  clover::par_ranged1(globals.context.queue, Range1d{0u, xrange1}, [=](int j) {
    field.cellx[j] = 0.5 * (field.vertexx[j] + field.vertexx[j + 1]);
    field.celldx[j] = dx;
  });

  clover::par_ranged1(globals.context.queue, Range1d{0u, yrange1}, [=](int k) {
    field.celly[k] = 0.5 * (field.vertexy[k] + field.vertexy[k + 1]);
    field.celldy[k] = dy;
  });

  clover::par_ranged2(globals.context.queue, Range2d{0u, 0u, xrange1, yrange1}, [=](const int i, const int j) {
