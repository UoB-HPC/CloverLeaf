...
global_variables initialise(parallel_ &parallel, const std::vector<std::string> &args) {
  ...
  auto &&[ctx, file] = create_context(args);
  if (parallel.boss) report_context(ctx);
...
int main(int argc, char *argv[]) {
  ...
  if (parallel.boss) {
    std::cout << std::endl << "Clover Version " << g_version << std::endl << "Task Count " << parallel.max_task << std::endl << std::endl;
  }
